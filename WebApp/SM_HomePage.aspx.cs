...
namespace WebApp
{
    public partial class SM_HomePage : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            table = new DataTable();
            adapter = new SqlDataAdapter("select m.id as MatchID, cr.rname as ClubRepresentative, host.cname as HostClub, guest.cname as GuestClub, m.startTime as StartTime, m.endTime as EndTime \r\n        from HostRequest h\r\n        inner join StadiumManager sm on h.stadiumManagerID = sm.id\r\n        inner join ClubRepresentative cr on h.clubRepresentativeID = cr.id\r\n        inner join Match m on h.matchID = m.id\r\n        inner join Club guest on guest.id = m.guestClubID\r\n    inner join Club host on host.id = m.hostClubID  where h.approved is null and sm.username = '" + Session["User"] + "'", conn);
            adapter.Fill(table);
            fillUITable(UnhandeledRequests, table);
        ...
        protected void AcceptBUTTON_Click(object sender, EventArgs e)
        {
            ...
            var GuestClub ="";
            var StartTime="";
            var MatchID = AcceptOrRejectID.Text;
